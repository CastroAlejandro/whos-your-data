$(document).ready(function () {
	...
	const checkParameters = () => {
		...
		window.location = `http://localhost:8080/search?platform=${platform}&genre=${selectedGenre}`;
	}
	...
	$('.userFave').on('click', (event) => {
		...
		addUserFavorite(gameId, gameTitle, gameRating, gameGenre);
	});

	function addUserFavorite(id, title, rating, genre) {
		console.log("adding favorites")
		console.log(`ID: ${id} | Title: ${title} | Rating: ${rating} | Genre: ${genre}`);
	...
	$('#searchKeywordButton').on('click', () => {
		let searchKeyword = $('#searchKeyword').val().trim();
		console.log('Term: ' + searchKeyword);
		window.location = `http://localhost:8080/keyword?search=${encodeURIComponent(searchKeyword)}`;
	});
});